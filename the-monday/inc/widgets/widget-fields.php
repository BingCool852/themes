...
function the_monday_widgets_show_widget_field( $instance = '', $widget_field = '', $athm_field_value = '' ) {
    ...
    $the_monday_postlist[0] = array(
        'value' => 0,
        'label' => '--choose--'
    );
    $arg = array( 'posts_per_page' => -1 );
    $the_monday_posts = get_posts( $arg );
    foreach ( $the_monday_posts as $the_monday_post ) {
        $the_monday_postlist[$the_monday_post->ID] = array(
            'value' => $the_monday_post->ID,
            'label' => $the_monday_post->post_title
        );
    };
    ...
    switch ( $the_monday_widgets_field_type ) {
        ...
        case 'selectpost' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id( $the_monday_widgets_name ); ?>"><?php echo $the_monday_widgets_title; ?>:</label>
                <select name="<?php echo $instance->get_field_name( $the_monday_widgets_name ); ?>" id="<?php echo $instance->get_field_id( $the_monday_widgets_name ); ?>" class="widefat">
                    <?php foreach ( $the_monday_postlist as $the_monday_single_post ) { ?>
                        <option value="<?php echo $the_monday_single_post['value']; ?>" id="<?php echo $instance->get_field_id( $the_monday_single_post['label'] ); ?>" <?php selected( $the_monday_single_post['value'], $athm_field_value ); ?>><?php echo $the_monday_single_post['label']; ?></option>
                    <?php } ?>
                </select>
        ...
        case 'upload' :
            ...
            if ( function_exists( 'wp_enqueue_media' ) ) {
                if ( ( $value == '') ) {
                    $output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Upload', 'the-monday' ) . '" />' . "\n";
                } else {
